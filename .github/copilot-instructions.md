...
- **`_events/` collection** – Events (auto-generated from scraper)
  ...
  - Front matter fields (YAML): `id`, `title`, `community`, `datePublished`, `dateIso`, `dateFr`, `timeFr`, `link`, `img`, `localImg`, `place` (optional), `placeAddr` (optional), `dateIsoEnd` (optional)
...
Create an HTML file in `_events/` following the naming convention `YYYY-MM-DD-{community-slug}-{event-id}.html`:
