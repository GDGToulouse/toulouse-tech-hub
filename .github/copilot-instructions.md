...
### Data Model
Data is organized by entity type with dedicated folders:

- **`events/`** - Events entity
  - `events/data/*.yml` - Individual event files (one per event)
  ...
  - `events/imgs/` - Event images (WebP format when possible)
  
- **`confs/`** - Conferences entity
  - `confs/data/confs.yml` - Major annual conferences definitions
  - `confs/imgs/` - Conference images

- **`groups/`** - Communities entity
  - `groups/data/groups.yml` - Community definitions
    - Each entry can have: `id`, `name`, `url`, `registration`, `description`, `social[]`
  - `groups/imgs/` - Community logos/images

- **`_data/`** - Jekyll data directory (contains symlinks to entity data for Jekyll compatibility)
  - Symlinks: `_data/events` → `events/data/`, `_data/confs.yml` → `confs/data/confs.yml`, `_data/groups.yml` → `groups/data/groups.yml`

- **`events-job.json`** (repo root) - Source-of-truth list generated by the scraper job; used to detect updates and to regenerate event data and images.
...
### Scraper Control
- To suppress a scraper-generated event, add a `.skip` file next to the YAML file (example: `events/data/2025-03-04-agile-meetup-305839478.yml.skip`).
...
### Image Management
- Event images are stored in `events/imgs/` with filename matching the event ID
- Conference images are in `confs/imgs/`
- Community images are in `groups/imgs/`
- Images should be WebP format when possible
- Cards use `aspect-ratio: 16/9` CSS for consistent display

### Event Lifecycle
1. **Automated**: Scraper job (`toulouse-tech-hub-job`) runs every 4 hours via workflow, scrapes Meetup/community sites, updates `events-job.json`, downloads images into `events/imgs/`, and generates event YAML files in `events/data/`
2. **Manual**: Create YAML file following the convention in `events/data/`, submit PR
...
Create `events/data/YYYY-MM-DD-{community}-{title}.yml`:
```yaml
id: 'unique-id'
title: 'Event Title'
...
localImg: events/imgs/unique-id.webp
description: >
  HTML description (can be multi-line)
```

### Adding a New Community
Add to `groups/data/groups.yml`:
```yaml
- id: slug
  name: Full Name
  url: https://website.com
  img: /groups/imgs/slug.jpg
  description: |
